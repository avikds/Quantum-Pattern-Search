...
The state space is the 8-dimensional tensor product Hilbert space $\mathcal{H}_8 = (\mathbb{C}^2)^{\otimes 3}$. Initializing the system in the ground state $|000\rangle$ and applying the three-qubit Hadamard transform $H^{\otimes 3} = H \otimes H \otimes H$ yields the equiprobable coherent superposition state $|s\rangle$:
$$|s\rangle = H^{\otimes 3} |000\rangle = \frac{1}{\sqrt{8}} \sum_{x \in \{0,1\}^3} |x\rangle = \frac{1}{\sqrt{8}} \big(|000\rangle + |001\rangle + \dots + |111\rangle\big)$$

In this initial state, the probability amplitude of every candidate pattern is identical:
$$a_x = \langle x | s \rangle = \frac{1}{\sqrt{8}}, \quad P(x) = |a_x|^2 = \frac{1}{8} = 12.50\% \quad \forall x \in \mathcal{S}$$

---

### 1.2 Two-Dimensional Invariant Subspace Geometry
Let $w \in \{0, 1\}^3$ represent the unique target pattern. We decompose $\mathcal{H}_8$ into an orthonormal planar basis consisting of the target state $|w\rangle$ and its orthogonal complement $|w^\perp\rangle$:
$$|w^\perp\rangle = \frac{1}{\sqrt{N - 1}} \sum_{x \ne w} |x\rangle = \frac{1}{\sqrt{7}} \sum_{x \ne w} |x\rangle$$

The state vectors satisfy $\langle w | w^\perp \rangle = 0$ and $\langle w | w \rangle = \langle w^\perp | w^\perp \rangle = 1$. The uniform superposition state $|s\rangle$ resides entirely in the real plane $\mathcal{H}_2 = \text{span}\{|w^\perp\rangle, |w\rangle\}$:
$$|s\rangle = \cos(\theta) |w^\perp\rangle + \sin(\theta) |w\rangle$$

The characteristic geometric half-angle $\theta$ is determined by the projection:
$$\sin(\theta) = \langle w | s \rangle = \frac{1}{\sqrt{N}} = \frac{1}{\sqrt{8}} \implies \theta = \arcsin\left(\frac{1}{\sqrt{8}}\right) \approx 0.361367 \text{ rad } (20.7048^\circ)$$
...
In Dirac notation, $U_w$ is the Householder reflection operator across the hyperplane orthogonal to $|w\rangle$:
$$U_w = I - 2|w\rangle\langle w|$$

Within the planar basis $\{|w^\perp\rangle, |w\rangle\}$, the matrix representation of $U_w$ is:
...
$$|a_w|^2 = |-a_w|^2 = \frac{1}{8}$$

---

### 1.4 The Grover Diffusion Operator ($U_s$)
The diffusion operator $U_s$ implements a Householder reflection across the uniform superposition state vector $|s\rangle$:
$$U_s = 2|s\rangle\langle s| - I = H^{\otimes 3} \big(2|000\rangle\langle 000| - I\big) H^{\otimes 3}$$

For an arbitrary state $|\psi\rangle = \sum_x a_x |x\rangle$ with mean amplitude $\mu = \frac{1}{N} \sum_{x} a_x$, the action of $U_s$ executes an **inversion about the mean**:
$$a_x \mapsto 2\mu - a_x$$

In the planar basis $\{|w^\perp\rangle, |w\rangle\}$, the matrix representation of $U_s$ is:
...
The probability of projecting onto the target state $|w\rangle$ upon computational basis measurement is:
$$P_k(w) = |\langle w | G^k | s \rangle|^2 = \sin^2((2k + 1)\theta)$$

---

### 1.6 Analytical Derivation of the Optimal Iteration Count ($R$)
To maximize $P_k(w)$, the total rotation angle $(2k + 1)\theta$ must be as close as possible to $\frac{\pi}{2}$:
...
| Iteration ($k$) | Cumulative Angle $(2k+1)\theta$ | Target Amplitude $\sin((2k+1)\theta)$ | Non-Target Amplitude | Analytical Target Probability $P_k(w)$ | Decimal Value |
|:---:|:---:|:---:|:---:|:---:|:---:|
...
The three-qubit multi-controlled phase gate $CCZ$ marks the all-ones state $|111\rangle$:
$$CCZ = I - 2|111\rangle\langle 111|$$

To mark an arbitrary state $|w\rangle = |w_2 w_1 w_0\rangle$, we define the bit-conditional preconditioning operator $V_w$:
$$V_w = \bigotimes_{i=0}^{n-1} X^{1 - w_i}$$

where $X^0 = I$ and $X^1 = X$. This operator maps $|w\rangle$ bijectively to $|111\rangle$:
$$V_w |w\rangle = |111\rangle, \quad V_w |x\rangle \ne |111\rangle \quad \forall x \ne w$$

Because Pauli-$X$ is Hermitian and unitary ($X = X^\dagger = X^{-1}$), $V_w$ is self-inverse: $V_w^\dagger = V_w$. Conjugating $CCZ$ by $V_w$ yields:
$$U_w = V_w^\dagger (CCZ) V_w = V_w \big(I - 2|111\rangle\langle 111|\big) V_w = I - 2 V_w |111\rangle\langle 111| V_w = I - 2|w\rangle\langle w|$$
...
Because native three-qubit gates are not universally available on hardware backends, the multi-controlled phase gate $CCZ$ is decomposed into a multi-controlled bit-flip ($CCX$ / Toffoli) conjugated by single-qubit Hadamard gates:
...
The diffusion operator $U_s = 2|s\rangle\langle s| - I$ reflects probability amplitudes across their arithmetic mean. Expanding $|s\rangle = H^{\otimes n} |0\rangle^{\otimes n}$:
$$U_s = H^{\otimes n} \big(2|0\rangle^{\otimes n}\langle 0|^{\otimes n} - I\big) H^{\otimes n}$$

We factor out a global sign from the interior reflection operator:
$$2|0\rangle^{\otimes n}\langle 0|^{\otimes n} - I = -\big(I - 2|0\rangle^{\otimes n}\langle 0|^{\otimes n}\big) = - X^{\otimes n} \big(I - 2|1\rangle^{\otimes n}\langle 1|^{\otimes n}\big) X^{\otimes n} = - X^{\otimes n} (CCZ) X^{\otimes n}$$
...
Furthermore, after $R = 2$ Grover iterations, the global phase vanishes identically:
...
The compiled circuit was executed on Qiskit's `AerSimulator` backend configured for shot-based sampling ($N_\text{shots} = 4096$) targeting pattern $|101\rangle$.
...
### 5.2 Single-Target Execution Results ($|101\rangle$)

| Basis State $|x\rangle$ | Measured Counts $C(x)$ | Empirical Probability $\hat{P}(x)$ | Theoretical Probability $P_\text{theory}(x)$ | Deviation $|\hat{P} - P_\text{theory}|$ |
|:---:|:---:|:---:|:---:|:---:|
| $|000\rangle$ | 33 | $0.81\%$ | $0.78\%$ | $+0.03\%$ |
| $|001\rangle$ | 25 | $0.61\%$ | $0.78\%$ | $-0.17\%$ |
| $|010\rangle$ | 26 | $0.63\%$ | $0.78\%$ | $-0.15\%$ |
| $|011\rangle$ | 30 | $0.73\%$ | $0.78\%$ | $-0.05\%$ |
| $|100\rangle$ | 21 | $0.51\%$ | $0.78\%$ | $-0.27\%$ |
| **$|101\rangle$ (Target)** | **3889** | **$94.95\% \pm 0.34\%$** | **$94.53\%$** | **$+0.42\%$** |
| $|110\rangle$ | 41 | $1.00\%$ | $0.78\%$ | $+0.22\%$ |
| $|111\rangle$ | 31 | $0.76\%$ | $0.78\%$ | $-0.02\%$ |
...
| Target State $|w\rangle$ | Identified State $|x^*\rangle$ | Execution Status | Empirical $P(w)$ | Theoretical $P(w)$ | Absolute Deviation | Total Variation Distance (TVD) |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| $|000\rangle$ | $|000\rangle$ | **PASSED** | $0.9399$ | $0.9453$ | $0.0054$ | $0.0083$ |
| $|001\rangle$ | $|001\rangle$ | **PASSED** | $0.9468$ | $0.9453$ | $0.0015$ | $0.0054$ |
| $|010\rangle$ | $|010\rangle$ | **PASSED** | $0.9473$ | $0.9453$ | $0.0020$ | $0.0093$ |
| $|011\rangle$ | $|011\rangle$ | **PASSED** | $0.9478$ | $0.9453$ | $0.0024$ | $0.0083$ |
| $|100\rangle$ | $|100\rangle$ | **PASSED** | $0.9463$ | $0.9453$ | $0.0010$ | $0.0054$ |
| $|101\rangle$ | $|101\rangle$ | **PASSED** | $0.9424$ | $0.9453$ | $0.0029$ | $0.0063$ |
| $|110\rangle$ | $|110\rangle$ | **PASSED** | $0.9512$ | $0.9453$ | $0.0059$ | $0.0098$ |
| $|111\rangle$ | $|111\rangle$ | **PASSED** | $0.9453$ | $0.9453$ | $0.0000$ | $0.0059$ |

- **Benchmark Accuracy**: $\mathbf{8 / 8 \text{ States (100.0\%)}}$
...
Because Grover's iterator $G$ executes a constant rotation of $2\theta \approx 41.41^\circ$ per iteration in the planar subspace $\text{span}\{|w^\perp\rangle, |w\rangle\}$, the probability amplitude follows a sinusoidal trajectory:
$$P_k(w) = \sin^2((2k + 1)\theta)$$

### Empirical Oscillation Data ($k \in \{0, 1, 2, 3, 4, 5\}$):

| Iteration ($k$) | Empirical $P_k(|101\rangle)$ | Theoretical $P_k(w)$ | Subspace Dynamics Regime |
|:---:|:---:|:---:|---|
...
At $k=3$, the state vector rotates past the target axis $|w\rangle$ toward $-|w^\perp\rangle$, proving empirically that increasing iteration counts beyond $R = \lfloor \frac{\pi}{4}\sqrt{N} \rceil$ induces destructive interference.
...
|---|:---:|:---:|:---:|
| **Worst-Case Queries** | $N = 8$ | $N = 8$ | $\mathbf{R = 2}$ |
| **Average Query Complexity** | $\frac{N+1}{2} = 4.5$ | $\frac{N+1}{2} = 4.5$ | $\mathbf{R = 2}$ |
| **Asymptotic Complexity** | $\mathcal{O}(N)$ | $\mathcal{O}(N)$ | $\mathbf{\mathcal{O}(\sqrt{N})}$ |
| **Success Probability (at $R=2$)** | $25.0\%$ (2 queries) | $25.0\%$ (2 queries) | $\mathbf{94.53\%}$ |
